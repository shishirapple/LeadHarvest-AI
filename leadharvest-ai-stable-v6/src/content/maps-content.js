...
(() => {
  ...
  async function processCard(card, href) {
    ...
    try {
      ...
      if (storage) {
        ...
        if (result.duplicate) {
          duplicates++;
          try { chrome.runtime.sendMessage({ type: 'RECORD_DUPLICATE' }); } catch (e) {}
          return false;
        }
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    switch (msg.type) {
      case 'SET_QUERY_IF_NEEDED': {
        // If the current Maps page doesn't match the task query, navigate to it
        const currentQ = getSearchQuery();
        const desiredQ = (msg.query || '').trim();
        if (desiredQ && currentQ !== desiredQ) {
          const searchUrl = `https://www.google.com/maps/search/${encodeURIComponent(desiredQ)}`;
